...
const Home = () => { 
  ...
  const filteredCars = cars.filter((car) => {
    if (!car || !car.prod_name || !car.type) {
      return false;
    }
  
    const isNameMatch = car.prod_name.toLowerCase().includes(search.toLowerCase());
    const isTypeMatch =
      type === "All" || (type === "Car" ? car.type !== "Motorcycle" : car.type === type);
            ...
            {filteredCars.map((car) => {
              return (
                <tr key={car.id}>
                  ...
                  <td>${car.price}</td>
                  <td>{car.type}</td> 
                  <td><Link className="view" to={`/detailshome/${car.id}`}>View Details</Link></td>
                </tr>
              );
            })}
          </tbody>
